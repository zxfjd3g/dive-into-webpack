...
例如项目中的 `AppComponent.js` 文件有如下代码：
```js
// 在页面被点击时 alert 出文字
...
```
现在修改 `AppComponent.js` 文件，把 `Hello,Webpack` 改成 `Hello,World`，模块替换后，点击页面后你会发现先后 alert 出了两个弹窗，分别会显示出两个不同的文字。
出现这个问题的原因是这段监听事件的代码被执行了多次，每次发生模块热替换时，新模块被加载用于替换老模块，这段监听事件的代码就会执行一次。

Webpack 为我们考虑到了这点，要解决以上问题，需要在新模块加载时移除掉之前的事件监听，相关代码如下：
```js
if (module.hot) {
