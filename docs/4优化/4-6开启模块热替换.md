### 开启模块热替换
除了刷新整个网页外，DevServer 还支持一种叫做模块热替换( Hot Module Replacement )的技术可在不刷新整个网页的情况下做到超灵敏的实时预览。
...
DevServer 默认不会开启模块热替换模式，要开启该模式，只需在启动时带上参数 `--hot`，完整命令是 `webpack-dev-server --hot`，启动后日志如下：
```
...
bundle.js.map  1.33 MB       0  [emitted]         main
 ...
 [171] ./node_modules/react/react.js 56 bytes {0} [built]
 [187] ./node_modules/react-dom/index.js 59 bytes {0} [built]
 [273] ./main.css 993 bytes {0} [built]
    + 262 hidden modules
webpack: Compiled successfully.
```
可以看出 `bundle.js` 代理客户端相关的代码包含六个文件：
```
 [119] (webpack)-dev-server/client?http://localhost:8080 5.83 kB {0} [built]
 [129] (webpack)-dev-server/client/socket.js 856 bytes {0} [built]
 [161] (webpack)-dev-server/client/overlay.js 3.6 kB {0} [built]
...
webpack: Compiled successfully.
```
DevServer 新生成了一个用于替换老模块的布丁文件 `0.ea11a51f97f2b52bca7d.hot-update.js`，同时在浏览器开发工具中也能看到抓包：
