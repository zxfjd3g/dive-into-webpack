...
在安装 Webpack 前请确保你的系统安装了5.0.0版本及以上的 [Node.js](https://nodejs.org)。

在开始给项目加入构建前你需要先新建一个 Web 项目，方式包括：

- 新建一个目录，再进入项目根目录执行 `npm init` 来初始化最简单的、采用了模块化开发的项目。
- 用脚手架工具 [Yeoman](http://yeoman.io) 直接快速的生成一个最符合你需求的项目。

#### 安装 Webpack 到本项目
要安装 Webpack 到本项目可按照你的需要选择以下一个命令运行：
```bash
...
虽然介绍了2种安装方式，但是推荐安装到本项目，原因是可防止不同项目依赖不同版本的 Webpack 而导致冲突。
