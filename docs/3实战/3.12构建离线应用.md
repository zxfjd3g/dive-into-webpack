...
离线应用是指通过离线缓存技术，让资源在第一次被加载后缓存在本地，下次在访问它时就直接返回本地的文件，就是没有网络连接。
...
由于 Service Workers 无法通过 注入 polyfill 去实现兼容，所以在你打算使用它前请先调查清楚你的网页的运行场景。
...
```
一旦这个脚本文件被加载，Service Workers 的安装就开始了。这个脚本会被安装到浏览器中，就算用户关闭了当前网页，它仍会存在。
也就是说第一次打开该网页时 Service Workers 的逻辑不会生效，因为脚本还没有被加载和注册，但是以后再次打开该网页脚本里的逻辑将会生效。

在 Chrome 中可以通过打开 chrome://inspect/#service-workers 链接来查看当前浏览器中所有注册了的 Service Workers。
...
##### 更新缓存
在某个时候你的文件需要更新和重新发布出去，如果这个文件被离线缓存了，那就需要 Service Workers 脚本中有对应的逻辑去更新缓存。
...
并且 `sw.js` 文件中的代表需要被缓存的文件的 URL 列表 `cacheFileList` 的值需要根据输出文件列表所对应的 URL 来决定。
...
安装成功后，在项目更目录下执行 `webpack-dev-server` 命令后，DevServer 将以 HTTPS 模式启动，并输出如下日志：
```
> webpack-dev-server
...
为了验证 Service Workers 和缓存生效了，可以通过 Chrome 的开发者工具来查看。
