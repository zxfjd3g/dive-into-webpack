...
所以现在执行 Webpack 构建只需要通过 `npm run build` 去实现。
...
可以看出要满足以上三个要求，需要定义三个不同的任务，其中任务3又依赖任务2。
...
```json
{
  "scripts": {
    ...
    "dist": "webpack --config webpack_dist.config.js",
...
- `dist` 代表构建出用于发布到线上去的代码，输出到 `dist` 目录中。
- `pub` 代表先构建出用于发布到线上去的代码，再同步 `dist` 目录中的文件到发布系统。所以在开发完后需要发布时只需执行 `npm run pub`。
