...
#### 引入问题
上一节[3.7为单页应用生成HTML](3-9为单页应用生成HTML.md)中只生成了一个 HTML 文件，但在实际应用中一个完整的系统不会把所有的功能都做到一个网页中，因为这会导致这个网页性能不佳。
...
> 本实例[提供项目完整代码](http://webpack.wuhaolin.cn/3.8管理多个单页应用.zip)
